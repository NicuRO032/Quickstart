...
@TeleOp(name="TeleOp_Alianta_Albastra")
@Config
public class TeleOpAlbastru extends OpMode {
    ...
    public double curretX;
    public double currety;
    public double curretAngle;

    public static Pose Start_Pose;
    private final Pose PARK_POSE  = new Pose(38.5, 36, Math.toRadians(0));
    private Pose Current_Pose;


    public PathChain getDynamicParkPath() {
        return follower.pathBuilder()
                .addPath(new BezierLine(follower.getPose(), PARK_POSE))
                .setLinearHeadingInterpolation(follower.getPose().getHeading(), PARK_POSE.getHeading())
                .build();
    }
    ...
    public void init() {
        ...
        follower = Constants.createFollower(hardwareMap);
         ...
         curretX = follower.getPose().getX();
         currety = follower.getPose().getY();
         curretAngle = follower.getPose().getHeading();

        if (Start_Pose  == null) {
            Start_Pose = new Pose(91, 91, Math.toRadians(47));
        }
        follower.setStartingPose(Start_Pose);
        Current_Pose = new Pose(curretX, currety, Math.toRadians(curretAngle));
    ...
    private void handleDriver2Controls() {
        ...
        if (driver2.wasJustPressed(GamepadKeys.Button.Y)) {
            PathChain freshPark = getDynamicParkPath();
            follower.followPath(freshPark, true);
        }
    ...
    private void sendTelemetry() {
        ...
        telemetry.addLine("----Coordonate---");
        telemetry.addData("X= " , curretX);
        telemetry.addData("Y= " , currety);
        telemetry.addData("Heading= " , curretAngle);
