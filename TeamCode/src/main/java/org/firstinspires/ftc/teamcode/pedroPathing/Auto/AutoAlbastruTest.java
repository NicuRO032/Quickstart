...
@Autonomous(name = "AUTO.big.blueTEST", group = "Pedro Pathing")
public class AutoAlbastruTest extends CommandOpMode {
    ...
    private final Pose START_POSE = new Pose(47.5, 133.5, Math.toRadians(45));
    ...
    public void buildPaths() {
        ...
        // 2. CICLUL 1: UNIT (Drumul spre colectare + Colectarea propriu-zisă)
        grab1Path = follower.pathBuilder()
                // Primul segment: de la scor la intrarea în zona de colectare
                .addPath(new BezierLine(SCORE_POSE, GRAB1_START_POSE))
                .setLinearHeadingInterpolation(SCORE_POSE.getHeading(), GRAB1_START_POSE.getHeading())
                // Al doilea segment: mișcarea lentă de colectare
                ...
                // LOGICA DE VITEZĂ ȘI INTAKE:
                // Pornim intake-ul când ajungem la GRAB1_START
                .addPoseCallback(GRAB1_START_POSE, () -> intake.setPower(-1), 7)
                // Scădem viteza la 0.4 exact înainte să ajungem la GRAB1_START (aprox finalul primului segment)
                .addParametricCallback(0.45, () -> follower.setMaxPower(0.4))
                // Resetăm viteza la 1.0 la finalul colectării (finalul întregului PathChain)
                .addParametricCallback(0.95, () -> follower.setMaxPower(1.0))
                .build();

        // 3. Traiectoria de scor 1
        ...
        grab2Path = follower.pathBuilder()
                .addPath(new BezierLine(SCORE_POSE, GRAB2_START_POSE))
                ...
                .addPoseCallback(GRAB2_START_POSE, () -> intake.setPower(-1), 7)
                .addParametricCallback(0.45, () -> follower.setMaxPower(0.4))
                .addParametricCallback(0.95, () -> follower.setMaxPower(1.0))
                .build();

        // 5. Traiectoria de scor 2
    ...
    public void run() {
        ...
        if (!autoStarted) {
            ...
            SequentialCommandGroup autoSequence = new SequentialCommandGroup(
                    ...
                    new ParallelRaceGroup(
                            new FollowPathCommand(follower, grab1Path, true), // Rulează tot lanțul de colectare
                            new WaitUntilCommand(carousel::allSlotsOccupied),
                            new WaitCommand(3000) // Siguranță: max 5 secunde dacă nu se umple
                    ),
